...
grid_search = GridSearchCV(rf_model, param_grid, cv=3, scoring='roc_auc', verbose=1)
...
cv_scores = cross_val_score(best_rf_model, X_train_pca, y_train, cv=cv, scoring='roc_auc')
...
y_pred_test = best_rf_model.predict(X_test_pca)
y_prob_test = best_rf_model.predict_proba(X_test_pca)[:, 1]
...
covid_predictions = best_rf_model.predict(X_covid_pca)

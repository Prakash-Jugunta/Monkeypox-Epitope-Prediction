...
# Step 6: Define SVM and RF Models
svm_model = SVC(probability=True, class_weight='balanced', kernel='rbf', random_state=42)
rf_model = RandomForestClassifier(n_estimators=100, random_state=42)

# Step 7: Create the Stacking Model (SVM + RF with Logistic Regression as Meta-Model)
stacked_model = StackingClassifier(
    estimators=[('svm', svm_model), ('rf', rf_model)],
    final_estimator=LogisticRegression()
)

# Step 8: Cross-Validation and Hyperparameter Tuning (if needed)
...
grid_search = GridSearchCV(
    stacked_model, param_grid, cv=3, scoring='roc_auc', verbose=1
)

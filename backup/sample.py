...
unseen_data = pd.read_csv('input_unseen.csv')  # Unseen dataset

# Handle missing values
bcell_data.fillna(bcell_data.mean(), inplace=True)
unseen_data.fillna(unseen_data.mean(), inplace=True)
...
explainer = shap.Explainer(best_svm.predict, X_train_pca)
shap_values = explainer(X_test_pca)

# SHAP Summary Plot
shap.summary_plot(shap_values, X_test_pca, feature_names=[f"PCA{i+1}" for i in range(X_test_pca.shape[1])])

# SHAP Force Plot for a specific instance
shap.force_plot(explainer.expected_value, shap_values[0, :], matplotlib=True)

# Prepare unseen data
X_unseen = unseen_data.drop(columns=['parent_protein_id', 'protein_seq', 'peptide_seq'], errors='ignore')

# Scale and apply PCA to unseen data
X_unseen_scaled = scaler.transform(X_unseen)
X_unseen_pca = pca.transform(X_unseen_scaled)

# Predict on unseen data
unseen_predictions = best_svm.predict(X_unseen_pca)

# Separate epitope and non-epitope predictions
epitopes = unseen_data[unseen_predictions == 1]
non_epitopes = unseen_data[unseen_predictions == 0]

# Save epitope and non-epitope regions
epitopes.to_csv('unseen_epitopes.csv', index=False)
non_epitopes.to_csv('unseen_non_epitopes.csv', index=False)

print("\nUnseen data predictions saved:")
print(" - Epitope regions saved to 'unseen_epitopes.csv'.")
print(" - Non-epitope regions saved to 'unseen_non_epitopes.csv'.")

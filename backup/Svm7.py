...
explainer = shap.Explainer(best_svm, X_train_pca)
shap_values = explainer(X_test_pca)

# Summary plot
shap.summary_plot(shap_values, X_test_pca, feature_names=[f"PCA{i}" for i in range(X_test_pca.shape[1])])

# 6. Cross-validation performance visualization
plt.figure(figsize=(6, 4))
